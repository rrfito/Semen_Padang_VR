...
const formatValue = (field, value) => {
    ...
    if (field === 'lat' || field === 'latitude' || field === 'lng' || field === 'longitude') {
...
export default function PendingChangesModal({ isOpen, onClose, onPublished }) {
    ...
    const fetchChanges = async () => {
        setLoading(true);
        try {
            const response = await axios.get(route('admin.editor.pending-changes'));
    ...
    const getEventColor = (event) => {
        switch(event) {
            case 'created': return { bg: 'bg-green-500/10', border: 'border-green-500/30', text: 'text-green-400', icon: 'add_circle' };
            case 'updated': return { bg: 'bg-amber-500/10', border: 'border-amber-500/30', text: 'text-amber-400', icon: 'edit' };
            case 'deleted': return { bg: 'bg-red-500/10', border: 'border-red-500/30', text: 'text-red-400', icon: 'delete' };
            default: return { bg: 'bg-slate-500/10', border: 'border-slate-500/30', text: 'text-slate-400', icon: 'circle' };
        }
    };
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
            <div className="bg-surface-dark rounded-xl shadow-2xl max-w-4xl w-full max-h-[85vh] overflow-hidden flex flex-col border border-border-dark">
                ...
                <div className="px-6 py-4 border-b border-border-dark flex items-center justify-between bg-[#15202b]">
                    <div>
                        <h2 className="text-xl font-bold text-white">📋 Pending Changes</h2>
                ...
                {!loading && (
                    <div className="px-6 py-4 bg-[#111a22] border-b border-border-dark grid grid-cols-4 gap-4">
                        <div className="text-center">
                            <div className="text-2xl font-bold text-purple-400">{data.summary.total_changes || 0}</div>
                            <div className="text-xs text-slate-400">Total Changes</div>
                        </div>
                        <div className="text-center">
                            <div className="text-2xl font-bold text-cyan-400">{data.summary.areas_count || 0}</div>
                            <div className="text-xs text-slate-400">Areas</div>
                        </div>
                        <div className="text-center">
                            <div className="text-2xl font-bold text-indigo-400">{data.summary.scenes_count || 0}</div>
                            <div className="text-xs text-slate-400">Scenes</div>
                        </div>
                        <div className="text-center">
                            <div className="text-2xl font-bold text-pink-400">{data.summary.links_count || 0}</div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    {loading ? (
                        ...
                        <div className="text-center py-12">
                            <span className="material-symbols-outlined text-6xl text-slate-700 mb-4 block">check_circle</span>
                            <p className="text-lg text-slate-400 font-medium">No Pending Changes</p>
                            <p className="text-sm text-slate-500 mt-2">All changes have been published</p>
                        </div>
                    ) : (
                        <>
                            {Object.entries(data.changes).map(([type, changes]) => (
                                <div key={type}>
                                    <h3 className="text-sm font-bold text-slate-400 uppercase mb-3 flex items-center gap-2">
                                        <span className="material-symbols-outlined text-lg">
                                            {type === 'Area' ? 'domain' : type === 'Scene' ? '360' : 'link'}
                                        </span>
                                        {type}s ({changes.length} changes)
                                    </h3>
                                    <div className="space-y-3">
                                        {changes.map(change => {
                                            const colors = getEventColor(change.event);
                                            return (
                                                <div key={change.id} className={`p-4 rounded-lg border ${colors.bg} ${colors.border}`}>
                                                    <div className="flex items-start gap-3">
                                                        <span className={`material-symbols-outlined ${colors.text} mt-0.5`}>
                                                            {colors.icon}
                                                        </span>
                                                        <div className="flex-1 min-w-0">
                                                            <div className="flex items-center justify-between gap-2">
                                                                ...
                                                                <div className="flex items-center gap-2 shrink-0">
                                                                    ...
                                                                    <span className={`text-xs font-bold uppercase px-2 py-1 rounded ${colors.bg} ${colors.text} border ${colors.border}`}>
                                                                        {change.event}
                                                                    </span>
                                                                </div>
                                                            </div>
                                                            <div className="text-xs text-slate-500 mt-1">
                                                                {change.edit_count > 1 ? (
                                                                    <>First edit {change.oldest_edit} · Latest {change.created_at} · by {change.causer_name}</>
                                                                ) : (
                                                                    <>{change.created_at} · by {change.causer_name}</>
                                                                )}
                                                            </div>
                                                            
                                                            {/* Show detailed changes for updates */}
                                                            {change.event === 'updated' && change.change_details && change.change_details.length > 0 && (
                                                                <div className="mt-3 space-y-2">
                                                                    {change.change_details.map((detail, idx) => (
                                                                        <div key={idx} className="text-xs bg-slate-900/50 rounded px-3 py-2 border border-slate-700">
                                                                            <div className="text-slate-400 font-bold mb-1">
                                                                                {getFieldLabel(detail.field)}:
                                                                            </div>
                                                                            <div className="flex items-center gap-2">
                                                                                <span className="text-red-400 line-through flex-1 break-all">
                                                                                    {formatValue(detail.field, detail.old)}
                                                                                </span>
                                                                                <span className="text-slate-600">→</span>
                                                                                <span className="text-green-400 flex-1 break-all">
                                                                                    {formatValue(detail.field, detail.new)}
                                                                                </span>
                                                                            </div>
                                                                        </div>
                                                                    ))}
                ...
                <div className="px-6 py-4 border-t border-border-dark flex items-center justify-between bg-[#111a22]">
                    <div className="text-xs text-slate-500">
                        {data.summary.oldest_change && <>Oldest: {data.summary.oldest_change}</>}
                    </div>
                    <div className="flex gap-3">
                        <button onClick={onClose} className="px-4 py-2 text-slate-400 hover:text-white transition-colors font-medium">
                            Close
                        </button>
                        <button 
                        ...
                        >
                            ...
                            <span>Publish All ({data.summary.total_changes})</span>
                        </button>
                    </div>
                </div>
            </div>
